...
首次可能要好幾天才補得完 —— **會自己接續，不必重跑**。

補的順序是**由新往舊**：三年 ≈ 2,200 次請求，遠超單日上限，中途被擋是常態。
由新往舊的話，任何時間點手上都是「一段到今天為止的連續區間」，只是比較短；
由舊往新則會留下一個中間的洞，而 5 日／20 日累計會橫跨那個洞，
**算出來的數字看起來正常但是錯的**。短而正確勝過長而破洞。

⚠️ **請替這台申請一把自己的 FinMind token。** 與其他服務共用的話，
這裡的回補會吃光當日配額，那個服務隔天早上就抓不到東西 ——
而失敗的樣子是「資料沒更新」，不會有人立刻聯想到是被這裡吃掉的。
`BACKFILL_MAX_PER_RUN`（預設 40）限制每輪的量，但那只是緩解，不是解法。
